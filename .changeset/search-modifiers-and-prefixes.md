...
- Number and quantity params get a prefix selector (`eq`/`ne`/`lt`/`le`/`gt`/`ge`/`ap`), intersected with the server's `SearchParameter.comparator` when advertised. Date params additionally offer the range boundaries `sa`/`eb`, but only when the target element covers a range (Period/Timing) — a scalar `date`/`dateTime` target (`birthdate`, …) doesn't. A prefix hydrated from a URL that the resolved target doesn't support is stripped so the submitted query stays valid.
