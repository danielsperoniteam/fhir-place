...
| `run-prompt-locally.sh` | Shared wrapper used by every driver. Handles lock, log, pause file, dirty-tree refusal, iMessage failure notification, `--add-dir` for worktree parent, and provider selection with `AGENT_PROVIDER={claude,codex,random}`. Claude remains the default and never gets `ANTHROPIC_API_KEY`, so it falls back to OAuth. |
