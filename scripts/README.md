...
| `poll-events.sh` | Long-lived daemon (60s loop) that watches GitHub for new issues, PRs, and slash commands (`/dispatch-engineer`, `/resolve-conflicts`, `/address-comments`). When `main` advances it updates clean same-repository PR branches and dispatches isolated conflict resolution for dirty ones. Durable per-head leases prevent duplicate runs. State lives under `~/.fhir-place-state/`. |
