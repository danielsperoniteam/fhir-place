...
| `/resolve-conflicts` comment on PR, or same-repository PR blocked by merge conflicts | `poll-events.sh` (local, every 60s); the GHA copy is disabled | `scripts/local/event-resolve-conflicts.sh` (local) | Local Claude (subscription) | Manual commands are collaborator-gated. Automatic resolution is limited to PR heads in this repository targeting `main`; forks remain read-only. |
...
| **2. Feature branch behind main** | `poll-events.sh` records the current `main` SHA. When it changes, the daemon calls GitHub's `update-branch` API for every clean same-repository PR targeting `main`, using the PAT so normal PR synchronize events and CI still fire. Conflicted branches go through the isolated resolver. Per-PR SHA leases prevent retry storms. Fork PRs are read-only. |
